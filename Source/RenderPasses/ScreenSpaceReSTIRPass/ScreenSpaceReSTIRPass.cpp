...
namespace
{
    ...
    const Falcor::ChannelList kOutputChannels =
    {
        ...
        { "color2",                 "gColor2",                  "Final color (dual)",       true /* optional */, ResourceFormat::RGBA32Float },
...
void ScreenSpaceReSTIRPass::execute(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    if (dict.keyExists("clearReservoir"))
    {
        for (int i = 0; i < mpScreenSpaceReSTIR.size(); i++)
            mpScreenSpaceReSTIR[i]->resetReservoirCount();
    }
    ...
    // Set the seed using mFrameCount
    else
    {
        for (int i = 0; i < mpScreenSpaceReSTIR.size(); i++)
        {
            mpScreenSpaceReSTIR[i]->setSeed(false, -1, false, -1, mFrameCount);
        }
    }
