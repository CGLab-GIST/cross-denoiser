...
namespace
{
    ...
    const Falcor::ChannelList kOutputChannels =
    {
        ...
        { kOutputColor2,                "gOutputColor2",                "Output color (linear)", true /* optional */ },
...
void ReSTIRPTPass::execute(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    dict["enableScreenSpaceReSTIR"] = mUseDirectLighting;

    static bool toBeTurnedOff = false;
    if (mSyncSeedSSReSTIR)
    {
        dict["syncSeedSSReSTIR"] = true;
        dict["fixSpatialSeed"] = mFixSpatialSeed;
        dict["spatialSeed"] = mSpatialSeed;
        dict["fixTemporalSeed"] = mFixTemporalSeed;
        dict["temporalSeed"] = mTemporalSeed;
        dict["temporalSeedOffset"] = mTemporalSeedOffset;
        toBeTurnedOff = true;
    }
    else if (toBeTurnedOff)
    {
        // Set this only once, to reset the seed of SSReSTIR
        dict["syncSeedSSReSTIR"] = false;
        toBeTurnedOff = false;
    }
...
bool ReSTIRPTPass::beginFrame(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    if (dict.keyExists("clearReservoir"))
    {
        mReservoirFrameCount = 0;
        // mParams.frameCount = 0; // Frame count keeps increasing for using different seeds for the multiple sequences (ref_restir)
        dict.removeKey("clearReservoir");
    }
